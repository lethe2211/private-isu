...
module Isuconp
  class App < Sinatra::Base
    ...
    IMAGE_DIR = File.expand_path('../../public/img', __FILE__)
    ...
    post '/' do
      ...
      if params['file']
        ...
        db.prepare(query).execute(
          me[:id],
          mime,
          '', # Will not store the binary to DB
          params["body"],
        )
        pid = db.last_id

        # Move the uploaded image file to /public/images/#{pid}.#{ext}
    ...
    get '/image/:id.:ext' do
      ...
      if (params[:ext] == "jpg" && post[:mime] == "image/jpeg") ||
        ...
        # Write the image file from DB as a static image file so that NGINX can distribute it from the next time (without accessing the application server)
